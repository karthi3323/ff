...
$items_query = "SELECT ei.*, p.name as product_name 
                ...
                WHERE ei.quotation_id = :quotation_id 
                ORDER BY ei.id ASC";
...
class QuotationPDF extends FPDF {
    ...
    function Footer() {
        ...
        // $this->Ln(3);
        /* $this->Cell(0, 5, 'This is a computer generated quotation', 0, 1, 'C');
    ...
    function addQuotationHeader($quotation_no, $quotation_date, $dispatch_from, $dispatch_through, $agent_name, $startY) {
        $this->SetXY(110, $startY);
        $this->Rect(5, 16, 200, 26);
        // Left column: quotation details
        /* $this->SetFont('Arial', 'B', 11);
        $this->SetTextColor($this->primary_color[0], $this->primary_color[1], $this->primary_color[2]);
        $this->Cell(80, 8, 'QUOTATION DETAILS', 0, 1); */
        $this->SetTextColor(0,0,0);
        
        $this->SetFont('Arial', '', 10);
        $this->SetX(110);
        $this->Cell(35, 6, 'Quotation No:', 0, 0);
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(45, 6, $quotation_no, 0, 1);
        
        $this->SetFont('Arial', '', 10);
        $this->SetX(110);
        $this->Cell(35, 6, 'Date:', 0, 0);
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(45, 6, ($quotation_date ? date('d-m-Y', strtotime($quotation_date)) : '-'), 0, 1);
    ...
    function addItemsTable($items, $quotation) {
        ...
        foreach ($items as $index => $item) {
            if ($count == $row_per_page) {
                ...
                $this->addPartyDetails(
                    ...
                    trim(($quotation['address_line1'] ?? '') . ' ' . ($quotation['address_line2'] ?? '')),
                    $quotation['p_place'],
                    $quotation['p_state'],
                    $quotation['p_gst']
                );
                ...
                $this->addQuotationHeader(
                    $quotation['quotation_no'],
                    $quotation['quotation_date'],
                    $quotation['dispatch_from'],
                    $quotation['dispatch_through'],
    ...
    function addTotalsSection($quotation, $total_cases) {
        $startY = $this->GetY();
        ...
        $this->Cell($cell_width + $value_width, 8, 'QUOTATION SUMMARY', 1, 1, 'C', true); */
        
        $this->SetColor(array(0,0,0));
        $this->SetFont('Arial', '', 10);
        
        // Helpers
        $goods_value = (float)$quotation['goods_value'];
        $discount_pct = (float)$quotation['discount_percent'];
        $discount_amt = $goods_value * ($discount_pct / 100);
        $sub_total = (float)$quotation['sub_total'];
        $packing_amt = $sub_total * ((float)$quotation['packing_percent'] / 100);
        $mahamai_amt = $sub_total * ((float)$quotation['mahamai_percent'] / 100);
        $insurance_amt = $sub_total * ((float)$quotation['insurance_percent'] / 100);
        $commission_amt = $sub_total * ((float)$quotation['commission_percent'] / 100);
        $taxable_amt = (float)$quotation['taxable_amount'];
        $sgst_amt = $taxable_amt * ((float)$quotation['sgst_percent'] / 100);
        $cgst_amt = $taxable_amt * ((float)$quotation['cgst_percent'] / 100);
        $igst_amt = $taxable_amt * ((float)$quotation['igst_percent'] / 100);
        ...
        if($quotation['packing_percent'] > 0) {
            $this->SetX($start_x);
            $this->Cell($cell_width, $summ_line, 'Packing Charges (' . (float)$quotation['packing_percent'] . '%):', 'LR', 0, 'L');
            $this->SetFont('Arial', 'B', 10);
            $this->Cell($value_width, $summ_line, number_format($packing_amt, 2), 'LR', 1, 'R');
            $this->SetFont('Arial', '', 10);
        }
        if($quotation['mahamai_percent'] > 0) {
            $this->SetX($start_x);
            $this->Cell($cell_width, $summ_line, 'Mahamai (' . (float)$quotation['mahamai_percent'] . '%):', 'LR', 0, 'L');
            $this->SetFont('Arial', 'B', 10);
            $this->Cell($value_width, $summ_line, number_format($mahamai_amt, 2), 'LR', 1, 'R');
            $this->SetFont('Arial', '', 10);
        }
        if($quotation['insurance_percent'] > 0) {
            $this->SetX($start_x);
            $this->Cell($cell_width, $summ_line, 'Insurance (' . (float)$quotation['insurance_percent'] . '%):', 'LR', 0, 'L');
            $this->SetFont('Arial', 'B', 10);
            $this->Cell($value_width, $summ_line, number_format($insurance_amt, 2), 'LR', 1, 'R');
            $this->SetFont('Arial', '', 10);
        }
        if($quotation['commission_percent'] > 0) {
            $this->SetX($start_x);
            $this->Cell($cell_width, $summ_line, 'Commission (' . (float)$quotation['commission_percent'] . '%):', 'LR', 0, 'L');
            $this->SetFont('Arial', 'B', 10);
        ...
        if($quotation['sgst_percent'] > 0) {
            $this->SetX($start_x);
            $this->Cell($cell_width, $summ_line, 'SGST (' . (float)$quotation['sgst_percent'] . '%):', 'LR', 0, 'L');
            $this->SetFont('Arial', 'B', 10);
        ...
        if($quotation['cgst_percent'] > 0) {
            $this->SetX($start_x);
            $this->Cell($cell_width, $summ_line, 'CGST (' . (float)$quotation['cgst_percent'] . '%):', 'LR', 0, 'L');
            $this->SetFont('Arial', 'B', 10);
        ...
        if($quotation['igst_percent'] > 0) {
            $this->SetX($start_x);
            $this->Cell($cell_width, $summ_line, 'IGST (' . (float)$quotation['igst_percent'] . '%):', 'LR', 0, 'L');
            $this->SetFont('Arial', 'B', 10);
        ...
        $this->Cell($value_width, $summary_height, CURRENCY . ' ' . number_format($quotation['net_amount'], 2), 'BR', 1, 'R');
        ...
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(50, 6, $quotation['dispatch_through'] ?: '-', 0, 1);
        
        $this->SetFont('Arial', '', 9);
        $this->Cell(30, 6, 'Transport:', 0, 0);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(50, 6, $quotation['transport_name'] ?: '-', 0, 1);
        
        $this->SetFont('Arial', '', 9);
        $this->Cell(30, 6, 'LR No.:', 0, 0);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(50, 6, $quotation['eway_bill_no'] ?: '-', 0, 1);
...
$pdf->setCompanyInfo(
    $quotation['company_name'] ?? (defined('COMPANY_NAME') ? COMPANY_NAME : 'Company Name'),
    $quotation['company_address'] ?? '',
    $quotation['company_city'] ?? '',
    $quotation['company_state'] ?? '',
...
foreach ($copies_to_print as $copy_name) {
    ...
    $pdf->SetX(5);
    $pdf->addItemsTable($items, $quotation);
    $pdf->addTotalsSection($quotation, $total_cases);
    $pdf->addAmountInWords((float)$quotation['net_amount']);
}

$filename = 'Quotation_' . preg_replace('/[^A-Za-z0-9\-]/', '_', $quotation['quotation_no']);
